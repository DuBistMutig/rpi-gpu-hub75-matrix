...
__attribute__((pure))
uint8_t
byte_gamma_correct(const uint8_t x, const float gamma);
...
__attribute__((pure))
Normal
normal_gamma_correct(const Normal x, const float gamma);
...
__attribute__((pure))
Normal
hable_tone_map(const Normal color);
...
__attribute__((pure))
Normal
reinhard_tone_map(const Normal color, const float level);
...
__attribute__((pure))
Normal
aces_tone_map(const Normal color);
 ...
 *
 */
__attribute__((cold)) void *tone_map_rgb_bits(const scene_info *scene, const int num_bits, float *quant_errors);
